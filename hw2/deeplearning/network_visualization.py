...
def compute_saliency_maps(X, y, model):
    ...
    ##############################################################################
    score = model(X)
    gathered_score = score.gather(1, y.view(-1, 1)).sum()
    gradient = torch.autograd.grad(gathered_score, X)[0]
    saliency = torch.max(torch.abs(gradient), dim=1)[0]
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return saliency
...
def make_fooling_image(X, target_y, model):
    ...
    for _ in range(100):
        ...
        if target_y == scores.data.max(dim=1)[1][0]:
            break
        score = scores[0, target_y]
        gradient = torch.autograd.grad(score, X_fooling)[0]
        X_fooling = X_fooling + learning_rate * gradient
...
def update_class_visulization(model, target_y, l2_reg, learning_rate, img):
    ...
    score = model(img)[0, target_y] - l2_reg * torch.sum(img ** 2)
    gradient = torch.autograd.grad(score, img)[0]
    img = img + learning_rate * gradient
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
    return img.detach()
