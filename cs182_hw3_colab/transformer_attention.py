...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        similarity =th.matmul(queries/th.sqrt(key_dim),keys.transpose(-2,-1))  # Compute the similarity according to the QKV formula
        ...
        weights = nn.Softmax(dim=-1)(masked_similarity) # Turn the similarity into a normalized output. Remember that the last dim contains the features
