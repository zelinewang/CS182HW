...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        self.params = {
            ...
            'W2': np.random.normal(0, weight_scale, size=((input_dim[1]//2) ** 2 * num_filters, hidden_dim)),
    ...
    def loss(self, X, y=None):
        ...
        scores, af_cache = affine_forward(out, W3, b3)         
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        weights_norm = 0
        for i in range(1, 4):
            weights_norm += (self.params['W{}'.format(i)]**2).sum()         
        loss += 0.5 * self.reg * weights_norm
        da, dw3, db3 = affine_backward(dx, af_cache) 
        da, dw2, db2 = affine_relu_backward(da, ar_cache) 
        da, dw1, db1 = conv_relu_pool_backward(da, crp_cache) 
        grads = {
